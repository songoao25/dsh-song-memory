...
function VersionDialog(props: { client: MnemonClient; writeEnabled: boolean; onClose: () => void; onRefreshStatus: () => void }): JSX.Element {
  ...
  return <SidebarModal title={t('versions.title')} description={t('versions.description')} busy={busy} onClose={props.onClose}>
    <div className={css.versionDialogBody}>
      ...
      {result !== null && <div className={css.versionResult} role="status"><strong>{result.updated ? t('versions.updated', { name: result.component === 'mnemon' ? 'Mnemon CLI' : 'dsh-song-memory' }) : t('versions.alreadyCurrent')}</strong>{result.restartRequired && <p>{t('versions.restartRequired')}</p>}</div>}
      {snapshot !== null && <div className={css.versionList}>{snapshot.components.map(component => {
        ...
        return <article key={component.id} data-outdated={component.outdated || undefined}>
          ...
          {component.id === 'dsh-song-memory' && component.installPath !== undefined && <small className={css.versionLocation} title={component.installPath}><span>{dshInstallLabel(t, component)}</span><code>{component.installPath}</code></small>}
  ...
  return (
    <div className={css.page}>
      ...
      <section className={css.healthStrip} aria-label={t('status.aria')}>
        <article><span className={`${css.healthIndicator} ${status === null ? css.healthMuted : css.healthGood}`} /><div><small>{t('status.engine')}</small><strong>{status?.dshMnemonVersion === undefined ? 'dsh-song-memory' : `dsh-song-memory ${status.dshMnemonVersion}`}</strong><p>{status === null ? t('status.pluginChecking') : t('status.pluginReady')}</p></div></article>
